...
static float nearestCarAhead(const std::vector<Car>& v, float x, float lane){
    float best=1e9f; for(auto&c:v) if(c.x>x+0.5f && std::fabs(c.lateral-lane)<0.40f) best=std::min(best,c.x-x); return best;
}
