...
void World::generate(uint32_t seed) {
    ...
    // ---- Obstacles ------------------------------------------------------
    // One obstacle per cluster, placed in one of 3 lanes, spaced ~34m apart.
    // Because only a single lane is blocked per stretch and spacing is wide,
    // there is ALWAYS a clear adjacent lane within steering reach -> the
    // course is always passable and never walls all three lanes at once.
    obstacles.clear();
    ...
    for (float x = startSprint + 30.0f; x < totalLength - 40.0f; x += obsStep) {
        ...
        if (lane == prevLane && rng.next_f() < 0.6f) lane = (lane + 1) % 3; // avoid monotone walls
        prevLane = lane;
        Obstacle o;
        o.x = x + rng.range(-5.0f, 5.0f);
    ...
    // blocking the same stretch of road.
    cars.clear();
